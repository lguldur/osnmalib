...
namespace
{
    ...
    void AppendDebugBit(std::vector<std::uint8_t>& out,
        int32_t& bit_count,
        bool bit)
    {
        const int32_t bit_in_byte = bit_count % 8;
    ...
    bool BuildDebugRangeBytes(const GalileoNavCandidate& candidate,
        int32_t wt,
        int32_t first_bit,
        int32_t bit_count,
        ...
        int32_t& out_bits)
    {
        ...
        if ((first_bit + bit_count) > GAL_INAV_BITS)
            return false;
        ...
        for (int32_t i = 0; i < bit_count; ++i)
        {
            AppendDebugBit(out,
                out_bits,
                GetBitMsb0(word.even.data(), first_bit + i));
        }

        return true;
    }

    bool BuildDebugAdkd0Bytes(const GalileoNavCandidate& candidate,
        std::vector<std::uint8_t>& out,
        int32_t& out_bits)
    {
        out.clear();
    ...
    void PrintAdkd0PayloadDebug(const char* label,
        const GalileoNavCandidate& candidate)
    {
        printf("%s prn=%d selected_wn=%d selected_tow=%.0f ",
            label,
            candidate.prn,
            candidate.creation_time.wn,
        ...
        static constexpr std::array<Range, 5> ranges =
        {
            Range{"wt1", GAL_WT1, 6, 120},
            Range{"wt2", GAL_WT2, 6, 120},
            Range{"wt3", GAL_WT3, 6, 122},
            Range{"wt4", GAL_WT4, 6, 120},
            Range{"wt5", GAL_WT5, 6, 67}
        };

        for (const Range& range : ranges)
        {
            std::vector<std::uint8_t> range_bytes;
            int32_t range_bits = 0;

            if (BuildDebugRangeBytes(candidate,
                range.wt,
                range.first_bit,
                range.bit_count,
                range_bytes,
                range_bits))
            {
                printf("%s_bits=%d ", range.name, range_bits);
                PrintHexPrefix(range.name,
                    range_bytes.data(),
                    static_cast<int32_t>(range_bytes.size()),
                    24);
                printf(" ");
            }
            else
            {
                printf("%s=missing ", range.name);
            }
        }

        std::vector<std::uint8_t> ced_bytes;
        int32_t ced_bits = 0;

        if (BuildDebugAdkd0Bytes(candidate,
            ced_bytes,
            ced_bits))
        {
            printf("ced_bits=%d ", ced_bits);
            PrintHexPrefix("ced549=",
                ced_bytes.data(),
                static_cast<int32_t>(ced_bytes.size()),
                96);
        }
        else
        {
            printf("ced549=missing");
        }
                ...
                key_size_bytes))
            {
                    ...
                    mac_input))
                {
                    ...
                    if (tag0_mismatch_debug_count < 40)
                    {
                        ...
                        static int32_t tag0_adkd0_payload_debug_count = 0;

                        if (tag0_adkd0_payload_debug_count < 20)
                        {
                            PrintAdkd0PayloadDebug("ADKD0 payload debug: stage=2",
                                *tag0_candidate);

                            ++tag0_adkd0_payload_debug_count;
                        }
    ...
    */
    for (int32_t i = 0; i < mack.tag_info_count; ++i)
    {
        ...
        if (tag_mismatch_debug_count < 160)
        {
            ...
            static int32_t tag_adkd0_payload_debug_count = 0;

            if ((tag.adkd == OsnmaAdkd::InavCed ||
                    tag.adkd == OsnmaAdkd::SlowMac) &&
                tag_adkd0_payload_debug_count < 40)
            {
                PrintAdkd0PayloadDebug("ADKD0 payload debug: stage=3",
                    *candidate);

                ++tag_adkd0_payload_debug_count;
            }
